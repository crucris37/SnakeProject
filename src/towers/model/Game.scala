...
class Game {
  ...
  def checkForBaseDamage(): Unit = {
    // TODO: Objective 1
    for((str, player) <- players){
      if(dist(player.location.x, player.location.y, level.base.x + .5 , level.base.y + .5) < playerSize){
        baseHealth -= 1
  ...
  def checkForPlayerHits(): Unit = {
    ...
    for((str, player) <- players){
      for(projectile <- projectiles){
        if(dist(player.location.x, player.location.y, projectile.location.x, projectile.location.y) < playerSize){
          projectile.destroy()
          player.location = startingVector()
        }
      }
