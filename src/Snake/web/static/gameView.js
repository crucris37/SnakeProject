...
function parseGameState(event) {
    ...
    for (let player of gameState['players']) {
        ...
        ctx.rect(parseFloat(player["x"]), parseFloat(player["y"]), 10, 10);
        ctx.stroke();
        for(let variable of player['body']){
            ctx.beginPath();
            ctx.lineWidth = "4";
            ctx.strokeStyle = player['color'];
            ctx.rect(parseFloat(variable["x"]), parseFloat(variable["y"]), 10, 10);
            ctx.stroke();
        }
    }
}
...
function initializeGame(inputUsername) {
    console.log("Hello From initializeGame");
...
// console.log(parseFloat(gameState["players"][0]['x']));
// console.log(parseFloat(gameState["players"][0]['y']));
// ctx.beginPath();
// ctx.lineWidth = "4";
// ctx.strokeStyle = "green";
// ctx.rect(parseFloat(gameState["players"][0]['x']), parseFloat(gameState["players"][0]['y']), 10, 10);
// ctx.stroke();


// placeCircle(20 ,20, "red", 10)
// context.beginPath();
